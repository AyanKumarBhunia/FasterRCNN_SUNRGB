...
def get_train_dataflow(src):
    ...
    textfile_index = natsorted([src + f for f in np.sort(os.listdir(src)) if f.endswith('.txt')])
    imgs = []
    count = 0
    for fn in textfile_index:
        ...
        each_file['is_crowd'] = np.zeros_like(each_file['class']).astype(np.int8)
