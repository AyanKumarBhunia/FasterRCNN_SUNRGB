...
# dataset -----------------------
_C.DATA.BASEDIR = './COCO_Dataset'
_C.DATA.TRAIN = []  # i.e., trainval35k ['train2014', 'valminusminival2014']
_C.DATA.VAL = []   # For now, only support evaluation on single dataset
_C.DATA.NUM_CATEGORY = 19    # 80 categories.
_C.DATA.CLASS_NAMES = 20  # NUM_CLASS (NUM_CATEGORY+1) strings, to be populated later by data loader. The first is BG.

# basemodel ----------------------
_C.BACKBONE.WEIGHTS = '/media/ayan/Drive/All_Object/tensorpack-master/FasterRCNN/ImageNet-ResNet50.npz'   # /path/to/weights.npz
...
def finalize_configs(is_training):
    ...
    if is_training:
        ...
        if _C.TRAINER == 'horovod':
            ...
            ngpu = get_num_gpu()
        assert ngpu % 3 == 0 or 3 % ngpu == 0, ngpu
